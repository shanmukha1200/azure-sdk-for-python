...
async def handle_invoke(request: Request) -> Response:
    ...
    if topic.lower() == "hang" or topic.lower().startswith("hang:"):
        seconds = 20.0
        if ":" in topic:
            try:
                seconds = float(topic.split(":", 1)[1])
            except ValueError:
                seconds = 20.0
        logger.critical(
            "🥶 HANG triggered via invoke — worker event loop will freeze for "
            "%.1fs (alive but unresponsive; supervisor readiness should report 424)",
            seconds,
        )
        _schedule_hang(seconds, delay=0.3)
        return JSONResponse(
            {"status": "hanging", "blocked_seconds": seconds}, status_code=202
        )
...
def _schedule_hang(seconds: float, delay: float = 0.3) -> None:
    """Freeze the worker event loop after the HTTP response has flushed.

    The worker stays ALIVE (no exit, so the supervisor does not restart it),
    but its event loop is blocked, so the supervisor's loopback readiness
    probe times out and the platform-facing ``/readiness`` returns 424 for
    the duration. After ``seconds`` the loop unblocks and readiness recovers.
    """
    async def _do_hang() -> None:
        await asyncio.sleep(delay)
        logger.critical("Worker freezing event loop now for %.1fs", seconds)
        time.sleep(seconds)  # intentional blocking sleep — freezes this worker
        logger.critical("Worker event loop unfroze after %.1fs", seconds)

    asyncio.get_event_loop().create_task(_do_hang())
